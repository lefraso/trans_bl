...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &     Ampd2(imax/8+1,jmax,kphys,N), phased2(imax/8+1,jmax,kphys,N),
     ...
     &     x(3), y(3), coef(3), yy, ymax, xad, z
      ...
      do i = 1, imax/8 + 1
        ...
        Amp_max_00(i) = coef(3)*ymax*ymax + coef(2)*ymax + coef(1)
        log_Amp_max(i,1) = dlog(Amp_max_00(i))
      end do
      do i = 5, imax/8 + 1 - 3
        alpha_x(i,1)=(      -log_Amp_max(i-3,1)+log_Amp_max(i+3,1)+
     &                 9.d0*(log_Amp_max(i-2,1)-log_Amp_max(i+2,1))-
     &                45.d0*(log_Amp_max(i-1,1)-log_Amp_max(i+1,1)) )/
     &              (60.d0*dx*8.d0)
      end do
     ...
     &                           ( (dble(f-1)*omega)**2 )
      ...
      do f = 1, N / 4 + 1
        do i = 2, imax/8 + 1
          ...
          call polcoe(x,y,coef)
          ymax         = -0.5d0*coef(2)/coef(3)
          Amp_max(i,f) = coef(3)*ymax*ymax + coef(2)*ymax + coef(1)
      ...
      do f = 2, N / 4 + 1
        do i = 2, imax/8 + 1
          ...
          call polcoe(x,y,coef)
          ymax           = -0.5d0*coef(2)/coef(3)
          Amp_maxd2(i,f) = coef(3)*ymax*ymax + coef(2)*ymax + coef(1)
     ...
     &               Amp_maxd2(i,f), alpha_xd2(i,f)
        ...
        do i = 201, 271, 10 ! 201 equivale a x =  9
          var = 100*((8*(i-1)*dx)+x0)
        ...
        do j = 1, jmax
          do i = 1, imax/8 + 1
            ...
            do p = 2, kphys/4
              ...
              wrs             = wr
              wis             = wi
              h1r             = c1*(datap(i,j,p1,f)+datap(i,j,p3,f))
              h1i             = c1*(datap(i,j,p2,f)-datap(i,j,p4,f))
              h2r             = -c1*(datap(i,j,p2,f)+datap(i,j,p4,f))
              h2i             = c1*(datap(i,j,p1,f)-datap(i,j,p3,f))
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine four1(dataff,isig,ii,jj,f,N)
