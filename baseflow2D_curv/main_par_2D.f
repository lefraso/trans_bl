...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &       uxbt(imax,jmax), uybt(imax,jmax), wzbt(imax,jmax), x,
     &       dwzdx(ptsx,jmax), ue, afil(ptsx), bfil(ptsx), cfil(ptsx),
     &       varg(ptsx,jmax), dvargdx(ptsx,jmax), Go(imax), kc, y, pi 
      common/dwdx/ dwzdx
...
!     m = beta_fs / (2.d0 - beta_fs)
      do i = 1, ptsx
        x = dble(i+shift-1)*dx + x0
        m   = beta_fs(i+shift-1) / (2.d0 - beta_fs(i+shift-1))
        ux(i,jmax) = ue * x**m
        duexmdx(i) = ue * m * x**(m - 1.d0)
      end do
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &            dv(ptsx,jmax),    varg(ptsx,jmax),
     &       dvargdx(ptsx,jmax),     ux2(ptsx,jmax),
     &        dux2dx(ptsx,jmax)
      common/vc/ varg, dvargdx
     ...
     &            + varg(i,j) * dux2dx(i,j) + ux2(i,j) * dvargdx(i,j)
