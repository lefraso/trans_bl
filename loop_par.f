...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 uxb(ptsx,jmax), uyb(ptsx,jmax), wzb(ptsx,jmax), dyya, dya
...
c    &                 - dya * lp_poi_coef(2,1) * alpha*uyb(ptsx,jmax)
c         endif 
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      a(jmax-1,3) = pp_poi_coef(3,1) + v_k2b2(k) * dyya
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &           lapv(ptsx,kfour), d2uydx2(ptsx,kfour)
      ...
      do k = 1, kfour
        do i = 1, ptsx
          lapv(i,k) = d2uydx2(i,k) + ( w_at_w_coef(3) * uy(i,1,k)
     ...
     &              / ( dyy * w_at_w_coef(1) ) + v_k2b2(k) * uy(i,1,k)
        end do
      end do
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      if (my_rank .lt. numproc) then
        ! Enviando as penultimas colunas
        do i = 1, 3
     ...
     &                141, MPI_COMM_WORLD, status, ierr)
      ...
      if (my_rank .gt. 0) then
        ! Recebendo as primeiras colunas
        call MPI_Recv(aux, 3, MPI_COMPLEX16, my_rank - 1,
