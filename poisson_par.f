...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      include 'comm.multi'
      integer j, mesh
      real*8 a(jmax,5), k2b2, qx, qdyya, add
      ...
      a(2,3) = sp_poi_coef(5,mesh)*qdyya+sp_poi_coef(2,mesh)*add
      a(2,4) = sp_poi_coef(6,mesh)*qdyya+sp_poi_coef(3,mesh)*add
      a(2,5) = sp_poi_coef(7,mesh)*qdyya

      do j = 3, v_ptsy(mesh) - 2
       qdyya  = 1.d0 /  v_dy2(j-2,mesh)
       a(j,1) = cp_poi_coef(4,mesh)*qdyya
       a(j,2) = cp_poi_coef(5,mesh)*qdyya+cp_poi_coef(1,mesh)*add
       a(j,3) = cp_poi_coef(6,mesh)*qdyya+cp_poi_coef(2,mesh)*add
       a(j,4) = cp_poi_coef(7,mesh)*qdyya+cp_poi_coef(3,mesh)*add
      ...
      a(j-1,3) = pp_poi_coef(3,mesh)*qdyya + add
      a(j-1,4) = pp_poi_coef(4,mesh)*qdyya
      a(j-1,5) = 0.d0
 
      a(j,1)   = lp_poi_coef(5,mesh) * qdyya
      a(j,2)   = lp_poi_coef(4,mesh) * qdyya
      a(j,3)   = lp_poi_coef(3,mesh) * qdyya + add 
     &          -lp_poi_coef(2,mesh) * dsqrt(qdyya)* 
     &           dsqrt(alpha*alpha-k2b2)
      a(j,4)   = 0.d0
      a(j,5)   = 0.d0
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      include 'comm.fourier'
      integer i, j, k, mesh
      real*8  si, alpha2, ady2
      complex*16 b, c, d, fc(ptsx,jmax,msh), sc(ptsx,jmax,msh)
      complex*16 fct(ptsx,jmax,msh)

      alpha2 =   dsqrt( alpha*alpha - v_k2b2(k) )
     ...
     &     + lp_poi_coef(5,mesh)*fc(i,j-2,mesh)
        d = v_k2b2(k) * fc(i,j,mesh)
        fct(i,j,1) = sc(i,j,mesh) + si * ( b / (12.d0 * v_dx2(mesh))
     &               + c / ady2 + d - lp_poi_coef(2,mesh)
     &               * alpha2 * fc(i,j,mesh) / dsqrt(ady2))
      end if
