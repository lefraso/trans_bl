 ...
 contains
  ...
  subroutine general(fpp_fs, eta_end_fs)

   implicit none
   real(kind=8), intent(out) :: fpp_fs, eta_end_fs
   integer                   :: j, cont, N_eta
   real(kind=8)              :: y(6), w_new, w_zero_n, zk, zkpw, delta_w, eta_adp, x, E, h, m
  ...
  subroutine baseflow_fs(fpp_fs)
   ...
   implicit none
   real(kind=8), intent(inout) :: fpp_fs
...
!    write(3,"(1x,4d17.9)") yad, uad, vad, vort
  ...
  subroutine delta_calculation(fpp_fs, eta_end_fs, delta)
  ...
  subroutine derivs (y, dydx)
   ...
   dydx(1) =  y(2)
   dydx(2) =  y(3)
   dydx(3) =  (-y(1) * y(3)) - (beta_fs * (1.d0 - y(2)**2)) 
   dydx(4) =  y(5)
   dydx(5) =  y(6)
   dydx(6) =  (-y(4)*y(3)-y(1)*y(6)) + (2.d0*beta_fs*y(2)*y(5))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine derivs2 (y, dydx)
   ...
   dydx(1) =   y(2)
   dydx(2) =   y(3)
!  dydx(3) = - y(1)*y(3)/(2.d0-beta_fs)-beta_fs*(1.d0-y(2)*y(2))/(2.d0-beta_fs)
